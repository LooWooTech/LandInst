...
CREATE ALGORITHM=UNDEFINED DEFINER=`loowootech`@`localhost` SQL SECURITY DEFINER VIEW `vcheck_transfer` AS select `checklog`.`ID` AS `ID`,`checklog`.`InfoID` AS `InfoID`,`checklog`.`UserID` AS `UserID`,`checklog`.`CheckType` AS `CheckType`,`checklog`.`CreateTime` AS `CreateTime`,`checklog`.`UpdateTime` AS `UpdateTime`,`checklog`.`Result` AS `Result`,`checklog`.`Note` AS `Note`,`transfer`.`TargetInstID` AS `TargetInstID`,`transfer`.`Mode` AS `Mode`,`member`.`RealName` AS `RealName`,`member`.`InstitutionID` AS `CurrentInstID` from ((`checklog` join `transfer` on((`checklog`.`InfoID` = `transfer`.`ID`))) join `member` on((`checklog`.`UserID` = `member`.`ID`))) where (`checklog`.`CheckType` = '2');
...
INSERT INTO `checklog` VALUES ('7', '1', '5', '2', '2014-07-28 18:57:49', null, null, null);
...
INSERT INTO `examresult` VALUES ('1', '5', '2', null, '2014-07-27 14:33:00');
INSERT INTO `institution` VALUES ('4', 'Institution NO1', '1', '1', '2014-07-26 15:38:44', null, null, null, null, null);
INSERT INTO `member` VALUES ('5', '郑良俊', '0', '32032219821027003X', '1982-10-27 00:00:00', '男', '3667144@qq.com', '18121783230', '1');
...
INSERT INTO `transfer` VALUES ('1', '5', '4', '4', '0001-01-01 00:00:00', '0', null);
